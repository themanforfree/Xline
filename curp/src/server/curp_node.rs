...
impl<C: 'static + Command, RC: RoleChange + 'static> CurpNode<C, RC> {
    ...
    pub(super) async fn wait_synced(
        ...
        if self.curp.is_shutdown() {
            return WaitSyncedResponse::new_error::<C>(&CommandSyncError::Shutdown)
                .map_err(Into::into);
        }
        let id = req.id()?;
...
impl<C: 'static + Command, RC: RoleChange + 'static> CurpNode<C, RC> {
    ...
    async fn bcast_vote(curp: &RawCurp<C, RC>, vote: Vote) {
        ...
        let voters = curp.cluster().voters_ids();
        let resps = curp
            .connects()
            .iter()
            .filter_map(|c| {
...
mod tests {
    ...
    async fn vote_will_not_send_to_learner_during_election() {
        ...
        let _ig = curp
            .apply_conf_change(vec![ConfChange::add_learner(
                learner_id,
                "address".to_owned(),
            )])
            .await;
        ...
        let mut mock_connect_learner = MockConnectApi::default();
        mock_connect_learner
            .expect_vote()
            .returning(|_, _| panic!("should not send vote to learner"));
        curp.set_connect(
            learner_id,
            ConnectApiWrapper::new_from_arc(Arc::new(mock_connect_learner)),
        );
        tokio::spawn(CurpNode::election_task(Arc::clone(&curp)));
        sleep_secs(3).await;
        assert!(curp.is_leader());
