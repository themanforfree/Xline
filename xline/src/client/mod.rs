...
use crate::{
    rpc::{
        DeleteRangeResponse, PutResponse, RangeResponse, Request, RequestOp, Response, ResponseOp,
    },
...
use self::kv_types::DeleteRangeRequest;
...
impl Client {
    ...
    /// # Panics
    ///
    /// Panics if `EtcdClient::connect` fails.
    #[inline]
    pub async fn new(
        leader_index: usize,
        ...
        .await
        .unwrap_or_else(|e| panic!("failed to connect to server: {}", e));
    ...
    /// parse `ResponseOp` into `Response`
    ...
    pub async fn put(&mut self, request: PutRequest) -> Result<PutResponse> {
        if self.use_curp_client {
            ...
            if let Response::ResponsePut(response) = Self::parse_response_op(cmd_res.decode()) {
                Ok(response)
            } else {
                Err(anyhow!("Receive wrong response type"))
            }
        } else {
    ...
    pub async fn range(&mut self, request: RangeRequest) -> Result<RangeResponse> {
        if self.use_curp_client {
            ...
            let cmd = Command::new(key_ranges, req_op.encode_to_vec(), propose_id);
            let cmd_res = self.curp_client.propose(cmd).await?;
            if let Response::ResponseRange(response) = Self::parse_response_op(cmd_res.decode()) {
                Ok(response)
            } else {
                Err(anyhow!("Receive wrong response type"))
            }
    ...
    pub async fn delete(&mut self, request: DeleteRangeRequest) -> Result<DeleteRangeResponse> {
        if self.use_curp_client {
            ...
            let cmd = Command::new(key_ranges, req_op.encode_to_vec(), propose_id);
            let cmd_res = self.curp_client.propose(cmd).await?;
            if let Response::ResponseDeleteRange(response) =
                Self::parse_response_op(cmd_res.decode())
            {
                Ok(response)
            } else {
                Err(anyhow!("Receive wrong response type"))
